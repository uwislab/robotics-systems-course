...
<quiz>
1) 在离散 PID 的实现中，为减少微分项对测量噪声的敏感性，通常采取的措施是：
- [ ] 增大 Ki
- [x] 在微分项上加入低通滤波
- [ ] 增大采样周期
- [ ] 使用更高分辨率 ADC

正确。微分项对高频噪声敏感，常在微分路径上加入一阶低通滤波以抑制噪声影响。
</quiz>

<quiz>
2) 关于积分风up（integrator windup），下列哪些措施可用来缓解？
- [x] 输出饱和时停止积分累积
- [x] 使用增量式 PID
- [ ] 增大 Kp
- [x] 使用 anti-windup 回退策略

正确。输出饱和时停止积分、增量形式可减小风up，anti-windup 回退策略直接限制或修正积分项都是有效的措施。
</quiz>

<quiz>
3) 关于 Ziegler–Nichols 闭环整定法，下列哪些说法正确？
- [x] 将 Ki、Kd 置零，增大 Kp 直到系统出现持续振荡
- [x] 记录临界增益 K_u 与振荡周期 T_u
- [x] 通常会得到较为激进的参数，导致较大超调
- [ ] 适合对超调敏感或需要高精度控制的场合

正确。Ziegler–Nichols 方法通常会得到较为激进的参数，导致较大超调，不适合对超调敏感或需要高精度控制的场合，只能作为初始整定参考。
</quiz>

<quiz>
4) 在示例的直流电机速度控制中，若系统在负载突增时出现较大的超调和振荡，可能的改进措施有哪些？
- [x] 降低 Kp 或增加 Kd，以提高阻尼
- [x] 引入前馈控制（根据负载或参考变化提供预补偿）
- [x] 对速度测量进行滤波并为微分项添加滤波器
- [ ] 增大 Ki 以消除稳态误差

正确。降低 Kp 或增加 Kd 以提高阻尼、引入前馈控制、对速度测量进行滤波都是有效的改进措施。
</quiz>
